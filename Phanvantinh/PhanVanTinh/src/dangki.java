...
public class dangki extends JFrame {
	...
	public dangki() {
		...
		textField_2 = new HintTextField("Năm - Tháng - Ngày ");
		...
		Checkbox checkbox_1 = new Checkbox("Nữ",false,sbgr);
		...
		JLabel lblNewLabel_8 = new JLabel("Mã học sinh");
					...
					&&textField_5.getText().length()!=0&&textField_6.getText().length()!=0){
							...
							if(tam==1) {
								try {
									...
									Connection conn =DriverManager.getConnection("jdbc:mysql://localhost:3306/sinhvien","root","");
									Statement stm =conn.createStatement();
									String sqlString = "INSERT INTO `student` (`MaHS`, `Hovaten`, `Lop`, `Gioitinh`, `Ngaysinh`, `Noio`) VALUES ('"+textField_6.getText()+"', '"+textField.getText()+"', '"+textField_1.getText()+"', 'Nữ', '"+textField_2.getText()+"', '"+textField_4.getText()+"')";
									stm.executeUpdate(sqlString);
									//stm.executeUpdate("insert into student values('"+textField_6.getText()+"','"+textField.getText()+"','"+textField_1.getText()+"','"+textField_1.getText()+"','"+textField_2.getText()+"','"+textField_4.getText()+"')");	
								}
